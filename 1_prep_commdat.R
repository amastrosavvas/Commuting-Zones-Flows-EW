...
# Read in lookups to harmonised geographies
lad_lad20x <- read.csv(file = "./rawdata/lad_lad20x.csv") 
...
# Bind commuting across years; Remove NI and Scotland; scale up 1991 data 
commdat[["dir annual by LAD"]] <- 
  commdat %>% 
  bind_rows() %>%
  filter(!grepl("^N|^95|^S|^60", LADCD_O), !grepl("^N|^95|^S|^60", LADCD_D)) %>%
...
commdat[["dir annual by LAD20X"]] <-
  commdat[["dir annual by LAD"]] %>%
  left_join(
    lad_lad20x, by = c("LADCD_O" = "LADCD")
  ) %>% 
  rename(LAD20XCD_O = LAD20XCD, LAD20XNM_O = LAD20XNM) %>%
  left_join(
    lad_lad20x, by = c("LADCD_D" = "LADCD")
  ) %>% 
